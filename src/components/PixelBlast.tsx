...
// Simplified test shader
const FRAGMENT_SRC_TEST = `
...
void main() {
  ...
  vec3 color = uColor * (0.5 + 0.5 * pattern);
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      console.log('PixelBlast: WebGL2 context created');
      ...
      console.log('PixelBlast: Renderer created with transparent background');
      ...
      console.log('PixelBlast: Canvas added to container');
      const uniforms = {
      ...
      try {
        ...
        console.log('PixelBlast: Shader material created successfully');
      } catch (error) {
        console.error('PixelBlast: Shader compilation failed:', error);
        // Fallback to simple material
        material = new THREE.MeshBasicMaterial({ color: 0xB19EEF, transparent: true, opacity: 0.5 });
        console.log('PixelBlast: Using fallback basic material');
      }
      ...
      const setSize = () => {
        ...
        console.log('PixelBlast: Size set to', w, 'x', h, 'pixelSize:', uniforms.uPixelSize.value, 'density:', uniforms.uDensity.value);
      };
      setSize();
      ...
      let raf = 0;
      const animate = () => {
        // Debug: Log first few animation calls
        if (uniforms.uTime.value === 0) {
          console.log('PixelBlast: First animate() call');
        }
        ...
        if (composer) {
          ...
          composer.render();
          // Debug: Log composer renders
          if (uniforms.uTime.value < 1) {
            console.log('PixelBlast: Composer rendering, time:', uniforms.uTime.value, 'density:', uniforms.uDensity.value);
          }
        } else {
          renderer.render(scene, camera);
          // Debug: Log first few renders
          if (uniforms.uTime.value < 1) {
            console.log('PixelBlast: Rendering frame, time:', uniforms.uTime.value, 'density:', uniforms.uDensity.value);
          }
        }
        raf = requestAnimationFrame(animate);
      };
      raf = requestAnimationFrame(animate);
      console.log('PixelBlast: Animation loop started');
      threeRef.current = {
        renderer,
        scene,
        camera,
      ...
      if (transparent) {
        ...
        console.log('PixelBlast: Set to transparent');
      } else {
        t.renderer.setClearColor(0x000000, 1);
        console.log('PixelBlast: Set to black background');
      }
    ...
    >
      ...
      <div 
        ...
        style={{
          ...
          `,
          backgroundSize: `${pixelSize * 3}px ${pixelSize * 3}px`,
