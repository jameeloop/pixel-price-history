...
-- Create a more restrictive policy that only allows viewing basic info without email
CREATE POLICY "Public can view limited upload info" 
ON public.uploads 
FOR SELECT 
...
-- 2. Recreate the uploads_public view without SECURITY DEFINER
-- First drop the existing view
DROP VIEW IF EXISTS public.uploads_public;

-- Create a new view that uses the mask_email function but without SECURITY DEFINER
...
-- 3. Create proper RLS policy for the uploads_public view
ALTER VIEW public.uploads_public SET (security_barrier = true);

-- Allow public read access to the masked view
CREATE POLICY "Public can view masked uploads" 
ON public.uploads_public 
FOR SELECT 
USING (true);

-- 4. Ensure the likes table has proper RLS policies
-- Update the likes policies to be more restrictive about deletion
...
-- 5. Ensure predictions table has proper constraints
-- Add a check to ensure predictions are within valid range
ALTER TABLE public.predictions 
ADD CONSTRAINT predictions_price_range_check 
CHECK (predicted_price >= 1 AND predicted_price <= 10000);

-- 6. Add audit logging for admin access
